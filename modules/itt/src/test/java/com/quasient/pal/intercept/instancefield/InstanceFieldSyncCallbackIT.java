...
public class InstanceFieldSyncCallbackIT extends AbstractInterceptIT {
  ...
  public void testSingleBeforeCallbackOnGet() throws Exception {
    ...
    // 5. Receive callbacks using getCallbacks()
  ...
  /**
   * Tests multiple BEFORE callbacks on instance field GET operation.
   *
   * <p>Registers a BEFORE intercept on getCounter, calls it multiple times (n=3), and verifies
  ...
  public void testMultipleBeforeCallbacksOnGet() throws Exception {
    ...
    // 4. Invoke getCounter multiple times which triggers callbacks
    logger.info("Invoking getCounter {} times which should trigger {} callbacks", n, n);
    for (int i = 0; i < n; i++) {
      invoke(
          messageBuilder.buildInstanceMethod(
              myPeerUuid,
    ...
    logger.info("getCounter invocations completed");
  ...
  /**
   * Tests multiple AFTER callbacks on instance field GET operation.
   *
   * <p>Registers an AFTER intercept on getCounter, calls it multiple times (n=3), and verifies
   * exactly 3 callbacks are received after the field gets.
   */
  @Test
  @Ignore
  ...
  /**
   * Tests multiple BEFORE callbacks on instance field PUT operation.
   *
   * <p>Registers a BEFORE intercept on setCounter, calls it multiple times (n=3), and verifies
   * exactly 3 callbacks are received.
   */
  @Test
  @Ignore
  public void testMultipleBeforeCallbacksOnPut() throws Exception {
    ...
    // 4. Invoke setCounter multiple times which triggers callbacks
    logger.info("Invoking setCounter {} times which should trigger {} callbacks", n, n);
    for (int i = 0; i < n; i++) {
      invoke(
          messageBuilder.buildInstanceMethod(
              myPeerUuid,
    ...
    logger.info("setCounter invocations completed");
  ...
  /**
   * Tests multiple AFTER callbacks on instance field PUT operation.
   *
   * <p>Registers an AFTER intercept on setCounter, calls it multiple times (n=3), and verifies
   * exactly 3 callbacks are received after the field puts.
   */
  @Test
  @Ignore
