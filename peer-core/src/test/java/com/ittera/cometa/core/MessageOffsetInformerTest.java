...
public class MessageOffsetInformerTest extends ZmqEnabledTest {
	...
	public void writeReplyNodes_existingReqNode() throws Exception {
		...
		AccessibleObject from = this.getClass().getDeclaredMethod("writeReplyNodes_existingReqNode", null);
		ExecMessage replyMessage = messageBuilder.buildReturnValue(peerUuid, null, from, null, false,
			requestUuid.toString());

		// add log request to directory
		CountDownLatch latch = new CountDownLatch(1);
		palDirectory.addLogRequestAsync(log.getName(), logRequest, (curatorFramework, curatorEvent) -> latch.countDown());
	...
	public void writeReplyNodes_reqNodeInitiallyMissing() throws Exception {
		...
		AccessibleObject from = this.getClass().getDeclaredMethod("writeReplyNodes_reqNodeInitiallyMissing", null);
		ExecMessage replyMessage = messageBuilder.buildReturnValue(peerUuid, null, from, null, false,
			requestUuid.toString());
