...
public class OutboundMsgTest extends ZmqEnabledTest {
	...
	public void send() {
		...
		OutboundMsg msg = new OutboundMsg(MessageType.ExecMessage, headers, execMessageUuid, followingMessageUuid, body);
	...
	public void build() {
		...
		OutboundMsg msg = new OutboundMsg(MessageType.InterceptRequest, headers, interceptMessageUuid, followingMessageUuid, body);
	...
	public void from() throws InvalidProtocolBufferException {
		...
		OutboundMsg msg1 = new OutboundMsg(MessageType.ExecMessage, headers, execMessageUuid, followingMessageUuid, body);
	...
	public void testEquals() {
		...
		OutboundMsg msg1 = new OutboundMsg(MessageType.InterceptRequest, headers, messageUuid,
			followingMessageUuid, body);

		// assert content-only equality
		assertThat(new OutboundMsg(MessageType.InterceptRequest, headers, messageUuid,
			followingMessageUuid, body), is(msg1));

		// different type
		assertThat(new OutboundMsg(MessageType.ExecMessage, headers, messageUuid,
			UUID.randomUUID(), body), is(not(msg1)));

		// different headers
		List<InternalHeader> otherHeaders = Arrays.asList(writeAhead, writeAhead); // just duplicate the header
		assertThat(new OutboundMsg(MessageType.InterceptRequest, otherHeaders, messageUuid,
			UUID.randomUUID(), body), is(not(msg1)));

		// different message UUID
		assertThat(new OutboundMsg(MessageType.InterceptRequest, headers, UUID.randomUUID(),
			followingMessageUuid, body), is(not(msg1)));
