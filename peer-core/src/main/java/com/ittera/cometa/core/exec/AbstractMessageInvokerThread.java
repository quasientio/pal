...
public abstract class AbstractMessageInvokerThread extends Thread {
	...
	protected static final Logger logger = LoggerFactory.getLogger(AbstractMessageInvokerThread.class);
	...
	AbstractMessageInvokerThread(ThreadGroup group, Runnable target, String name, ZContext zmqContext,
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Initialized message invoker thread named: {} with dealerAddress: {}", name, dealerAddress);
		}
	}
	 ...
	 */
	AbstractMessageInvokerThread(ZContext zmqContext, MessageBuilder messageBuilder, String dealerAddress,
	...
	protected void closeConnections() {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Stopped invoker thread: {}, dispatched={} dismissed={}",
				getName(), requestsDispatched.get(), requestsDismissed.get());
		}
	}
	...
	private ExecMessage dispatch(ExecMessage requestMsg, @Nullable Long recordOffset) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Invoker dispatched request message uuid: {} and recordOffset: {}, reply uuid: {}",
	...
	private void dispatch(InterceptRequest interceptMsg, Long recordOffset) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Invoker dispatched request message uuid: {} and recordOffset: {}, reply: {}",
				interceptMsg.getMessageUuid(), recordOffset, result);
		}
