...
public abstract class ConnectedService extends AbstractService {
	...
	protected ConnectedService(UUID peerUuid, ZContext zmqContext, String syncSocketAddress,
														 ThreadGroup serviceThreadGroup, String serviceName) {
	...
	private final Thread createRunThread() {
		...
		t.setUncaughtExceptionHandler((thread, throwable) -> logger.error("Uncaught error in Service thread", throwable));
		return t;
	}
}
