 ...
 * of the ProxyDispatcher interface, so that the aspect (and the peer-aj module) is not directly
 * dependent on the *Dispatcher classes (in the core module), imported by AspectProxyDispatcher.
 * Therefore, peer-aj only needs to import peer-common. This avoids a circular dependency at
 * runtime, where peer-core depends on peer-aj (to classload the aspect)
 */
