...
public class PALDirectory {
	...
	public void registerPeer(UUID peerUuid, Properties peerProperties) throws Exception {
		...
		if (curator.checkExists().forPath(getPeerPath(peerUuid)) == null) {
			...
			logger.info("Skipping registration of existing peer with uuid: {} and properties: {}",
				peerUuid, peerProperties);
		}
	}
	...
	public LogInfo registerLog(String logName) throws Exception {
		...
		String createdNode = curator.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT)
	...
	public LogInfo newLog(String logNamePrefix) throws Exception {
		UUID newLogUuid = UUID.randomUUID();
		String createdNode = curator.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT_SEQUENTIAL)
	...
	public LogInfo getLastLog(String logNamePrefix) throws Exception {
		List<String> logs = curator.getChildren().forPath(getLogsPath()).stream()
			.filter(l -> l.startsWith(logNamePrefix)).collect(Collectors.toList());
	...
	public boolean logRequestExistsAsync(String logName, UUID requestUuid, BackgroundCallback callback,
																			 CuratorWatcher watcher) throws Exception {
		String requestNode = String.format("%s/%s", getLogPath(logName), requestUuid);
		return curator.checkExists().usingWatcher(watcher).inBackground(callback).forPath(requestNode) != null;
	}
	...
	public void deleteLogRequestAsync(String logName, LogRequest logRequest) throws Exception {
		...
		logger.info("Async-deleted w/o guarantee request node {} and its {} reply nodes, for log: {}", logRequest.getUuid(),
			replyNodes, logName);
	}
	...
	public List<String> getRequestNodeChildrenAsyncWithWatch(String logName, UUID requestUuid,
		...
		return curator.getChildren().usingWatcher(curatorWatcher).inBackground(backgroundCallback).forPath(requestNode);
	}

	//</editor-fold>

	//<editor-fold desc="Log replies">
	public void addLogReplyAsync(String logName, LogReply logReply, BackgroundCallback callback) throws Exception {
		...
		if (curator.checkExists().forPath(requestNode) != null) {
			String sb = "from" + KEYVALUE_SEP + logReply.getPeerUuid() +
				'\n' + "offset" + KEYVALUE_SEP + logReply.getOffset() + '\n';
			curator.create().inBackground(callback).forPath(newReplyNode, sb.getBytes(getEncodingCharset()));
			if (logger.isDebugEnabled()) {
	...
	private byte[] peerPropsToData(Properties peerProperties) {
		...
		if (peerProperties != null && !peerProperties.isEmpty()) {
			...
			for (String propKey : peerProperties.stringPropertyNames()) {
				sb.append(propKey.trim()).append(KEYVALUE_SEP).append(peerProperties.getProperty(propKey).trim());
	...
	/**
	 * Since we use the same Zookeeper node as Kafka does, we can directly peek into Kafka brokers info
	 */
	private void loadBrokerInfoSet() {
		...
		try {
			for (String brokerId : curator.getChildren().forPath(BROKERS_PATH)) {
				...
				String nodeData = new String(curator.getData().forPath(brokerInfoPath), getEncodingCharset());
				if (logger.isDebugEnabled()) {
